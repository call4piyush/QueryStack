...
import com.querystack.dto.Login;
import com.querystack.dto.Question;
import com.querystack.dto.Solution;
import com.querystack.dto.User;

public class DAOImplementationBack extends HibernateDaoSupport implements
		DAOService {
	...
	void insertDefaultQuestions(long userId) {
		...
		tagList.add("JEE");
		for (int i = 0; i < 25; i++) {
			quesList.add(new Question(userId, "Admin",
					(int) (Math.random() * 100), (int) (Math.random() * 100),
					"This is question title.",
					"This is question explaination.", tagList, new Date()));
			// sol.add(new Solution(1000+i, 100+i, userId, "This is solution.",
			// (new Date()).toString(), (int)(Math.random()*100),
			// (int)(Math.random()*100)));
			// sol.add(new Solution(2000+i, 100+i, userId, "This is solution.",
			// (new Date()).toString(), (int)(Math.random()*100),
			// (int)(Math.random()*100)));
		}
	...
	public User createAccount(User user) {
		...
		getHibernateTemplate().save(
				new Login(user.getEmailId(), user.getPassword()));
	...
	public boolean checkLogin(String email, String password) {
		// TODO Auto-generated method stub
		System.out.println(this.getClass().getSimpleName() + " EmailId: "
				+ email + " Pass: " + password);
		List<Login> login = getHibernateTemplate().find(
				"from Login l where l.emailId=? and l.password=?", email,
				password);
		if (login.size() > 0)
			return true;
		return false;
	...
	public List<Question> getQuestionByDefault() {
		...
		if (flag) {
			...
			badges.add(new Badges(2, "Premium"));
			badges.add(new Badges(1, "Super"));
			List<Long> favList = new ArrayList<Long>();
			favList.add((long) 1);
			favList.add((long) 2);
			getHibernateTemplate().save(
					new User("admin@admin.com", "Admin", "admin", (new Date())
							.toString(), badges, favList));
	...
	public List<Question> getMyQuestions(long userId) {
		// TODO Auto-generated method stub
		return (List<Question>) getHibernateTemplate().find(
				"from Question q where q.userId=?", userId);
	}

	@Override
	public Question getQuestionById(long quesId) {
		// TODO Auto-generated method stub
		List<Question> myQues = getHibernateTemplate().find(
				"from Question m where quesId=?", quesId);
		return myQues.get(0);
	}
	...
	public List<Solution> setMySolution(Solution solObj) {
		...
		List<Solution> list = getHibernateTemplate().find(
				"from Solution s where s.quesId=?", solObj.getQuesId());
		return list;
	}
	...
	public List<Solution> getSolutionsByQuesID(long qId) {
		// TODO Auto-generated method stub
		List<Solution> list = (List<Solution>) getHibernateTemplate().find(
				"from Solution where quesId=?", qId);
		return list;
	}
	...
	public User update(User user) {
		...
		Query q = s
				.createQuery("update User set name=:name, emailId=:emailId, dob=:dob where userId=:userId");
		...
		// List<User> u =
		// getHibernateTemplate().find("from User where userId=?",
		// user.getUserId());
		return getUserByEmailId(user.getEmailId());
	}
	...
	public int setVote(int i, long quesId) {
		...
		Session s = sf.openSession();
		Query q = s
				.createQuery("update Question set votes=votes+:i where quesId=:quesId");
		q.setInteger("i", i);
		q.setLong("quesId", quesId);
		q.executeUpdate();
		return DataAccessUtils.intResult(getHibernateTemplate().find(
				"select votes from Question where quesId=" + quesId));
	}
	...
	public int setGetSolVote(int i, long solId) {
		...
		Session s = sf.openSession();
		Query q = s
				.createQuery("update Solution set votes=votes+:i where solId=:solId");
		q.setInteger("i", i);
		q.setLong("solId", solId);
		q.executeUpdate();
		return DataAccessUtils.intResult(getHibernateTemplate().find(
				"select votes from Solution where solId=" + solId));
	}

	@Override
	public List<Solution> getMySolutions(long userId) {
		// TODO Auto-generated method stub
		return getHibernateTemplate().find("from Solution where userId=?",
				userId);
	}
	...
	public List<Question> getMyFavQuestions(List<Long> favQues) {
		...
		for (int i = 0; i < favQues.size(); i++) {
			List<Question> list2 = getHibernateTemplate().find(
					"from Question where quesId=?", favQues.get(i));
