...
<title>Welcome to QueryStack</title>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-xs-12 col-md-12">
				<h3>${key.session.name }</h3>
				<hr>
			</div>
			<div class="col-md-12">
				<div class="col-md-3">
					<img alt="${key.session.emailId}"
						src="resources/images/default_image.jpg" class="thumbnail" />
				</div>
				<div class="col-md-9">
					<span>Name:</span><span>${key.session.name}</span><br> <span>Email
						ID:</span><span>${key.session.emailId }</span><br> <span>Date
						of Birth:</span><span>${key.session.dob }</span><br> <span>Reputation:</span><span>${0}</span><br>
					<hr>
					<span><a href="update.spring"
						class="btn btn-default btn-primary">Update Account</a></span>
				</div>
			</div>
		</div>
		<hr>
								...
								<div id="panel1">
									...
									<ul class="list-unstyled">
										<c:forEach var="list" items="${key.list }">
											<li><span class="badge">${list.votes }</span> <span><b>Votes</b></span>
												<a href="solution.spring?quesId=${list.quesId}">
													${list.quesTitle}</a></li>
										</c:forEach>
									</ul>
								</div>
								<div id="panel2">
									<ul class="list-unstyled">
										<c:forEach var="solList" items="${key.solList }">
											...
											<li><span class="badge">${solList.votes }</span> <span><b>Votes</b></span>
												<a href="solution.spring?quesId=${solList.quesId}">
													${solList.solution}</a></li>
										</c:forEach>
									</ul>
								</div>
								<div id="panel3">
									<ul class="list-unstyled">
										<c:forEach var="badge" items="${key.session.badges }">
											<li><span class="badge">${badge.badgeId }</span> <span><b>${badge.badgeName}</b></span>
											</li>
										</c:forEach>
									</ul>
								</div>
								<div id="panel4">
									...
									<ul class="list-unstyled">
										<c:forEach var="myFavQues" items="${key.myFavQuesList}">
											<!--  <li class="list-group-item"> -->
											<li><span class="badge">${myFavQues.votes }</span> <span><b>Votes</b></span>
												<a href="solution.spring?quesId=${myFavQues.quesId}">
													${myFavQues.quesTitle}</a></li>
										</c:forEach>
									</ul>
...
--%>
	...
	<div class="container">
		<hr />
		<p class="text-muted credit">&copy;All rigth reserved. Query Stack
			:)</p>
	</div>
	<!-- /Footer -->
